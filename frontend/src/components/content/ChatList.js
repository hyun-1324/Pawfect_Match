...
const ChatList = () => {
  ...
  const { loggedIn, statuses, login, sendJsonMessage, lastJsonMessage } =
    useAuth();
      ...
      Promise.allSettled(
        chatList.map((room) => {
          return fetchFromEndpoint(
            `http://localhost:3000/users/${room.user_id}`,
            { signal }
          )
            .then(({ data, error }) => {
              if (error) {
        ...
        const sortedRoomInfo = chatList
          .map((room) => roomInfoMap.get(room.id))
          .filter(Boolean);
        ...
        <div key={room.id} className="card userCard">
          ...
          <div className="nameAndButtons">
            ...
            <div className="buttonContainer notificationImageContainer">
              <button
                className="button userCardButton"
              ...
              {room.unReadMessage && (
                <div id="chatNotification" className="notificationMark"></div>
              )}
            </div>
          </div>
        </div>
    ...
    <div>
      ...
      {!errorMessage && (
        <div className="twoColumnCard">
          <div className="chatList">{makeChatlist()}</div>
          <div className="oneColumnCardLeft">
            <img
              className="chatImage"
              src={`${process.env.PUBLIC_URL}/images/chatListDog.png`}
              alt="dog with paper airplane"
            />
          </div>
        </div>
      )}
